...
const { uploadAttachments, getFileUrl } = require('../middleware/upload');
const { sendNotificationEmail } = require('../services/emailService');

async function addActivity(request, action, user, options = {}) {
  const activity = {
    action, performedBy: user._id, performedByName: user.name,
    performedByRole: user.role, fromStatus: options.fromStatus,
    toStatus: options.toStatus, comment: options.comment, timestamp: new Date()
  };
  ...
  if (options.notifyUsers && options.notifyUsers.length > 0) {
    const notifs = options.notifyUsers.map(uid => ({
      recipient: uid, type: options.notifType || 'status_change',
      title: options.notifTitle || `Request ${request.requestNumber} Updated`,
      message: options.notifMessage || action, relatedRequest: request._id
    }));
...
router.get('/', authenticate, async (req, res) => {
  const { status, city, dateFrom, dateTo, search, page = 1, limit = 20 } = req.query;
  const filter = {};
...
router.post('/', authenticate, authorize('marketplace', 'admin'),
  ...
  async (req, res) => {
    ...
    await addActivity(request, 'Request created', req.user, {
      toStatus: 'created', notifyUsers: misTeam.map(u => u._id),
      notifType: 'request_created',
      notifTitle: `New Branding Request: ${data.title}`,
      notifMessage: `A new branding request has been created by ${req.user.name}.`
    });
    await request.save();
    ...
    for (const misUser of misTeam) {
      sendNotificationEmail(misUser.email, 'New Branding Request',
        `A new branding request "${data.title}" was created.\nRequest: ${request.requestNumber}`
      ).catch(console.error);
    }
...
router.put('/:id/approve', authenticate, authorize('admin'), async (req, res) => {
  ...
  await addActivity(request, `Request ${decision} by Admin`, req.user, {
    fromStatus, toStatus: decision, comment, notifyUsers,
    notifType: decision, notifTitle: `Request ${decision.toUpperCase()}`,
...
router.put('/:id/assign-vendor', authenticate, authorize('mis', 'admin'), async (req, res) => {
  ...
  await addActivity(request, `Assigned to Vendor: ${vendor.name}`, req.user, {
    ...
    notifMessage: `You have been assigned work order ${request.requestNumber}.`
  });
  await request.save();
