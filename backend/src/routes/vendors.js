...
const { uploadVendor, getFileUrl } = require('../middleware/upload');
...
router.put('/:requestId/start', authenticate, authorize('vendor', 'admin'), async (req, res) => {
  ...
  request.activities.push({
    action: 'Vendor started branding work', performedBy: req.user._id,
    performedByName: req.user.name, performedByRole: req.user.role,
    fromStatus: 'assigned_to_vendor', toStatus: 'work_in_progress', timestamp: new Date()
  });
  await request.save();
...
router.put('/:requestId/complete', authenticate, authorize('vendor', 'admin'),
  ...
  async (req, res) => {
    ...
    request.activities.push({
      action: 'Branding work completed by vendor', performedBy: req.user._id,
      performedByName: req.user.name, performedByRole: req.user.role,
      fromStatus, toStatus: 'work_completed', timestamp: new Date()
    });
    const notifyIds = [request.createdBy, request.assignedMIS].filter(Boolean);
    await Notification.insertMany(notifyIds.map(uid => ({
      recipient: uid, type: 'work_completed',
      title: `Work Completed: ${request.requestNumber}`,
      message: `The branding work for request ${request.requestNumber} has been completed.`,
      relatedRequest: request._id, priority: 'high'
    })));
    await User.findByIdAndUpdate(req.user._id, { $inc: { 'vendorDetails.completedJobs': 1 } });
    await request.save();
